/**
 * Production contact API for Cloudflare Pages.
 * Mirrors `app/api/contact.ts` — Zod validates the body, Turnstile is verified,
 * then the lead is forwarded to the backend.
 *
...
import { contactApiSchema } from "../../lib/schemas/contact";
...
export async function onRequestPost(context) {
	...
	const parsed = contactApiSchema.safeParse(raw);
	if (!parsed.success) {
		return jsonError(formatZodError(parsed.error), 400);
	}

	const { name, email, details, turnstileToken } = parsed.data;
	...
	const backendResponse = await fetch(
		`${backendBase}/landing-page/leads/contact`,
		{
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
	...
	if (!backendResponse.ok) {
		return jsonError(
			`Contact lead submission failed (${backendResponse.status})`,
