...
export const useTextProcessingWorker = (options: UseWebWorkerOptions = {}) => {
	...
	const processAnimationSequence = useCallback(
		async (
			text: string,
			config: {
	...
	const splitText = useCallback(
		async (text: string, splitBy: "words" | "characters" | "lines") => {
			try {
				setState((prev) => ({ ...prev, isLoading: true, error: null }));
				...
				setState((prev) => ({
					...
					isLoading: false,
					error:
						error instanceof Error ? error.message : "Text splitting failed",
				}));
				throw error;
			}
		},
...
export const useDataProcessingWorker = (options: UseWebWorkerOptions = {}) => {
	...
	const filterProjects = useCallback(
		async (projects: unknown[], filters: Record<string, unknown>) => {
			try {
				setState((prev) => ({ ...prev, isLoading: true, error: null }));
...
export const useMathProcessingWorker = (options: UseWebWorkerOptions = {}) => {
	...
	const processMapDots = useCallback(
		async (dots: unknown[], config?: Record<string, unknown>) => {
			try {
				setState((prev) => ({ ...prev, isLoading: true, error: null }));
				...
				setState((prev) => ({
					...
					isLoading: false,
					error:
						error instanceof Error ? error.message : "Map processing failed",
				}));
				throw error;
			}
		},
	...
	const projectMapPoint = useCallback(
		async (lat: number, lng: number, width?: number, height?: number) => {
			try {
				setState((prev) => ({ ...prev, isLoading: true, error: null }));
	...
	const optimizeAnimationTimings = useCallback(
		async (animations: unknown[], targetFPS?: number) => {
			try {
				setState((prev) => ({ ...prev, isLoading: true, error: null }));
