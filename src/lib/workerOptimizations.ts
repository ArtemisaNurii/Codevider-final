	...
	try {
		...
		await Promise.all([
			manager.createWorker(
				"textProcessor",
				WORKER_CONFIGS.textProcessor.scriptPath,
			),
			manager.createWorker(
				"dataProcessor",
				WORKER_CONFIGS.dataProcessor.scriptPath,
			),
			manager.createWorker(
				"mathProcessor",
				WORKER_CONFIGS.mathProcessor.scriptPath,
			),
		]);
...
export class AdaptiveWorkerStrategy {
	...
	shouldUseWorker(operation: string): boolean {
		...
		const avgMain =
			history.main.reduce((sum, time) => sum + time, 0) / history.main.length;
		const avgWorker =
			history.worker.reduce((sum, time) => sum + time, 0) /
			history.worker.length;
