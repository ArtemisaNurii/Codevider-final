...
class WebWorkerManager {
	...
	async createWorker(workerType: string, scriptPath: string): Promise<void> {
		...
		try {
			...
			throw new Error(
				`Failed to create worker ${workerType}: ${(error as Error).message}`,
			);
		}
	}
	...
	async executeOperation<T = unknown>(
		...
		const cacheKey = enableCaching
			? this.getCacheKey(workerType, operation, data)
			: null;
		...
		if (!worker) {
			throw new Error(
				`Worker ${workerType} not found. Make sure to create it first.`,
			);
		}
	...
	private waitForWorkerReady(workerType: string): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			const timeout = setTimeout(() => {
				reject(
					new Error(`Worker ${workerType} failed to initialize within timeout`),
				);
			}, 10000);
