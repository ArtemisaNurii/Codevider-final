	...
	({
		...
		const TextComponent = (
			<div className={`p-8 md:p-10 ${bgColor} ${textColor}`}>
				...
				<h3 className="text-3xl font-sans font-bold mb-4">{title}</h3>
...
const ProjectPage: React.FC = memo(() => {
	...
	const [filteredStudies, setFilteredStudies] =
		useState<CaseStudy[]>(allCaseStudies);
	...
	useEffect(() => {
		const loadCategories = async () => {
			try {
				const result = (await processCategories(allCaseStudies)) as {
					categories: string[];
				};
				...
				setCategories([
					"All",
					...Array.from(new Set(allCaseStudies.map((c) => c.tag))),
				]);
			}
		};
		...
		const filterStudies = async () => {
			try {
				if (selectedCategory === "All") {
					...
					const result = await filterProjects(allCaseStudies, {
	...
	const rightColumnCases = filteredStudies.filter(
		(_, index) => index % 2 !== 0,
	);
	...
	return (
		<div className="bg-white font-sans will-change-scroll">
			...
			>
				...
				>
					Dive into our case studies to see how we have tackled unique
					challenges, crafted tailored strategies, and delivered impactful
					results across industries. Each story highlights our process from
					insight and design to execution and measurable success.
				</p>
