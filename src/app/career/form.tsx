		...
		if (files && files[0]) {
			...
			try {
				...
				const uploadData = uploadResult.data as
					| { fileMetadata: FileUploadData; candidateData: CandidateData }
					| undefined;
				if (uploadData) {
					const { candidateData, fileMetadata } = uploadData;
					setFileMetadata(fileMetadata);
					// Convert CandidateExperience[] to Experience[]
					const convertedExperiences = candidateData.experiences?.map(
						(exp) => ({
					...
					setSubmitMessage(
						"Resume processed successfully! Fields auto-filled.",
					);
				}
			} catch {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const submissionResult =
				await submitApplicationAction(applicationPayload);
			...
			<div className="w-full bg-gradient-to-r from-black via-slate-700 to-sky-600 py-30">
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					...
					<p className="mt-2 text-lg text-gray-200">
						...
						{job.addresses?.[0]?.address?.location || "Remote"}
					</p>
				</div>
			</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
					<div className="lg:col-span-2">
						...
						<form onSubmit={handleSubmit} className="mt-10 space-y-6">
							...
							{submitMessage && (
								<div
									className={`p-2 ${submitMessage.includes("Error") ? "text-red-600" : "text-green-600"}`}
								>
									{submitMessage}
								</div>
							)}
							<Button
								type="submit"
								disabled={isSubmitting || isProcessingResume}
								className="w-full"
							>
						...
						<div className="border border-gray-200 rounded-lg p-6">
							...
							<ul className="space-y-6 mt-4">
								...
								<JobDetailItem
									...
									title="Location"
									value={
										job.addresses?.[0]?.address?.location || "Not specified"
									}
								/>
								<JobDetailItem
									icon={FileText}
