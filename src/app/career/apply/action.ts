...
function extractCandidateData(
	...
	if (typeof responseData.full_name === "string")
		candidateData.name = responseData.full_name;
	if (typeof responseData.email === "string")
		candidateData.email = responseData.email;
	if (typeof responseData.phone === "string")
		candidateData.phone = responseData.phone;
	if (typeof responseData.date_of_birth === "string")
		candidateData.date_of_birth = responseData.date_of_birth;
	if (typeof responseData.gender === "string")
		candidateData.gender = responseData.gender as
			| "male"
			| "female"
			| "other"
			| "prefer_not_to_say";
	if (typeof responseData.bio === "string")
		candidateData.summary = responseData.bio;
	if (typeof responseData.cover_letter === "string")
		candidateData.cover_letter = responseData.cover_letter;

	if (Array.isArray(responseData.skills)) {
		candidateData.skills = responseData.skills.filter(
			(s: string) => typeof s === "string",
		);
	}
	if (Array.isArray(responseData.experiences)) {
		candidateData.experiences = responseData.experiences.map(
	...
	if (Array.isArray(responseData.educations)) {
		candidateData.educations = responseData.educations.map(
	...
	if (Array.isArray(responseData.projects)) {
		candidateData.projects = responseData.projects.map(
	...
	const candidate = responseData.candidate as
		| Record<string, unknown>
		| undefined;
	if (candidate && typeof candidate === "object") {
		if (!candidateData.name && typeof candidate.full_name === "string")
			candidateData.name = candidate.full_name;
		if (!candidateData.email && typeof candidate.email === "string")
			candidateData.email = candidate.email;
		if (!candidateData.phone && typeof candidate.phone === "string")
			candidateData.phone = candidate.phone;
		if (!candidateData.summary && typeof candidate.summary === "string")
			candidateData.summary = candidate.summary;
		if (!candidateData.skills && Array.isArray(candidate.skills)) {
			candidateData.skills = candidate.skills.filter(
...
export async function submitApplicationAction(
	...
	console.log(
		"Client action: submitApplicationAction called with data",
	...
	try {
		...
		if (response.ok) {
			...
			return {
				success: false,
				message: responseData.message || "Application submission failed",
			};
		}
	} catch (error) {
