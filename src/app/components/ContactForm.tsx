...
// Function to save lead to Notion database via API route
async function saveLeadToNotion(
  name: string,
  email: string,
  details: string
) {
  console.log(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/leads/landing/contact`);  

  const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/leads/landing/contact`, {
    method: "POST",
    body: JSON.stringify({
      name,
      email,
