    ...
    return (
        <>
            {richText.map((text, i) => {
                ...
                if (annotations.strikethrough) element = <span className="line-through">{element}</span>;
                if (annotations.underline) element = <span className="underline">{element}</span>;
                if (annotations.code) {
                    element = (
                ...
                if (annotations.color && annotations.color !== "default") {
                    element = <span className={colorMap[annotations.color]}>{element}</span>;
                }
                if (href) {
                    element = (
                        <a
...
export function RenderBlock({ block, depth = 0 }: { block: NotionBlock; depth?: number }) {
    const { type } = block;
    ...
    switch (type) {
        case "paragraph":
            return (
                <p className={`mb-4 leading-relaxed ${content.color ? colorMap[content.color] : "text-slate-600"}`}>
        ...
        case "heading_3": {
            ...
            return (
                <Tag className={`${size} font-bold text-slate-900 mt-10 mb-4 first:mt-0`}>
                    <RenderRichText richText={content.rich_text} />
                </Tag>
            );
        }
        ...
        case "numbered_list_item":
            return (
                <li className="text-slate-600 mb-1.5 ml-1">
                    <RenderRichText richText={content.rich_text} />
                    {block.has_children && <div className="ml-5 mt-2">{renderChildren()}</div>}
                </li>
            );
        ...
        case "callout":
            return (
                <div className="flex gap-4 p-5 mb-6 bg-white border border-slate-200 rounded-2xl shadow-sm items-center">
                    {content.icon?.emoji && <span className="text-2xl">{content.icon.emoji}</span>}
        ...
        case "image": {
            ...
            return (
                <figure className="my-10">
                    <img src={src} alt="Notion visual" className="rounded-2xl w-full object-cover border border-slate-200 shadow-md" />
        ...
        <div className="notion-content w-full max-w-4xl mx-auto">
            {groupedBlocks.map((group, index) => {
                ...
                if (group.type === "bulleted_list_item") {
                    return (
                        <ul key={`bullet-${index}`} className="list-disc mb-6 space-y-2 pl-5">
                ...
                if (group.type === "numbered_list_item") {
                    return (
                        <ol key={`numbered-${index}`} className="list-decimal mb-6 space-y-2 pl-5">
                            {group.items.map((block: NotionBlock) => (
                                <RenderBlock key={block.id} block={block} />
                            ))}
                        </ol>
